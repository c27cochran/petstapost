...
function convertLinks($message) {
    $parsedMessage = preg_replace(array('/(?i)\b((?:https?:\/\/|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:\'".,<>?«»“”‘’]))/', '/(^|[^a-z0-9_])@([a-z0-9_]+)/i', '/(^|[^a-z0-9_])#([a-z0-9_]+)/i'), array('<a href="$1" rel="nofollow">$1</a>', '$1<a class="comment-link" href="yappy.html?user=$2">@$2</a>', '$1<a class="comment-link" href="hashtag.html?hashtag=$2">#$2</a>'), $message);
...
if (isset($_GET['user'])) {
    ...
    $possible_friends = $friends->you_may_know($my_user_id);
    // Friend request data
    $fr_data = $notifications->get_friend_request_notifications($my_user_id);
    // Friend accepted data
    $accepted_data = $notifications->get_friend_accepted_notifications($my_user_id);
    // Comment data
    $comm_count = $notifications->count_comment_notifications($my_user_id);
    $comm_data = $notifications->get_comment_notifications($my_user_id);
    // Dig data
    $fav_data = $notifications->get_favorite_notifications($my_user_id);

    $my_username = $_GET['user'];
    // Comment mentions
    $comm_mention_data = $notifications->get_comment_mention_notifications($my_username);
    // Caption mentions
    $caption_mention_data = $notifications->get_mention_notifications($username);


echo '<div id="notification-container" class="white-popup-block" style="max-width:600px; margin: 20px auto;">';
    ...
    echo '<h2 class="notify-head" style="margin: 20px 30px; text-align: center;">No new notifications</h2>';
    ...
    if (!empty($possible_friends)) {
        ...
        echo '<h2 class="notify-head" style="margin: 20px 30px;">You may know...</h2>';
    ...
    if (!empty($accepted_data)) {
        ...
        echo '<h2 class="notify-head" style="margin: 20px 30px;">New Friends!</h2>';
    ...
    if (!empty($fr_data)) {
        ...
        echo '<span id="my-user-id" class="hidden">'.$my_user_id.'</span>';
        echo '<span id="my-full-name" class="hidden">'. $userdata['first_name'].' '. $userdata['last_name'].'</span>';

        echo '<h2 class="notify-head" style="margin: 20px 30px;">New Friend Requests!</h2>';
...
echo '<script type="text/javascript">
    $("#accept-friend-'.$notify_id.'").on("click", function(e) {
        ...
        $.ajax({
            type: "POST",
            url: "http://pestapost.com/util/accept-friend",
            data: "my_id="+my_id+"&their_id="+their_id+"&my_name="+my_name+"&their_name="+their_name,
            success: function(msg){
                span.parent().remove();
                message.removeClass("hidden");
                message.html(msg);
                p.html("Check out your new <a href="kibble.html?user='.$_GET["user"].'" class="logo light comment-link">Kibble!</a>");
            },
            error: function() {
    ...
    if (!empty($comm_mention_data) || !empty($caption_mention_data)) {
        ...
        echo '<h2 class="notify-head" style="margin: 20px 30px;">New Mentions!</h2>';
            ...
            foreach ($comm_mention_data as $comm) {
                ...
                if (!empty($first)) {
                    echo '<p class="popup-comment notification-section">
                            <a href="petstapost.html?item_id='.$item_id.'">
    ...
    if (!empty($comm_count)) {
        ...
        echo '<h2 class="notify-head" style="margin: 20px 30px;">Comments on Your Posts!</h2>';
    ...
    if (!empty($fav_data)) {
        ...
        echo '<h2 class="notify-head" style="margin: 20px 30px;">People <i class="fa fa-heart"></i> Your Posts!</h2>';

        foreach ($fav_data as $fav) {

            $fav_item = $notifications->get_favorite_item_notifications($my_user_id, $fav['liked_item']);

            $fav_count = $notifications->count_favorite_notifications($my_user_id, $fav['liked_item']);

            if ($fav_count == 1) {

                echo '<p class="popup-comment" style="margin: 20px 30px 0;">
                        <a href="petstapost.html?item_id='.$fav_item[0]["item_id"].'">
                            <img src="'.$fav_item[0]['url'].'" class="comm-img '.$fav_item[0]['filter'].'">&nbsp
                        </a>
                      </p>';

                echo '<p class="popup-comment" style="margin: 0 30px 0;">
                        <a class="comment-link" href="yappy.html?user='.$fav_item[0]['username'].'">'.$fav_item[0]['first_name'].' ' . $fav_item[0]['last_name'].'</a>';
            ...
            if ($fav_count == 2) {
                ...
                echo ' and <a href="yappy.html?user='.$fav_item[1]['username'].'" class="comment-link">'.$fav_item[1]['first_name'].' ' . $fav_item[1]['last_name'].'</a>';
            ...
            if ($fav_count > 2 && $fav_count < 12) {
                $count = ($fav_count-2);
                echo '<p class="popup-comment two-favorites notification-section" style="margin: 20px 30px 0;">
                        <a href="petstapost.html?item_id='.$fav_item[0]["item_id"].'">
                            <img src="'.$fav_item[0]['url'].'" class="comm-img '.$fav_item[0]['filter'].'" style="clear:right; float: left;">&nbsp;
                        </a>
                      </p>';

                echo '<p class="popup-comment two-favorites notification-section" style="margin: 0 30px 0;">
                        <a class="comment-link" href="yappy.html?user='.$fav_item[0]['username'].'">'.$fav_item[0]['first_name'].' ' . $fav_item[0]['last_name'].'</a>';

                echo ', <a href="yappy.html?user='.$fav_item[1]['username'].'" class="comment-link">'.$fav_item[1]['first_name'].' ' . $fav_item[1]['last_name'].'</a>';
                ...
                echo '<p class="popup-comment all-favorites hidden notification-section" style="margin: 20px 30px 0;">
                        <a href="petstapost.html?item_id='.$fav_item[0]["item_id"].'">
                            <img src="'.$fav_item[0]['url'].'" class="comm-img '.$fav_item[0]['filter'].'">&nbsp;<br><br>
                        </a>
                      ';

                $new_fav_count = ($fav_count-1); 
                for ($i=0; $i<$new_fav_count; $i++) {
                    echo '<a class="comment-link" href="yappy.html?user='.$fav_item[$i]['username'].'">'.$fav_item[$i]['first_name'].' ' . $fav_item[$i]['last_name'].'</a>, ';
                }
                    echo 'and <a class="comment-link" href="yappy.html?user='.$fav_item[$new_fav_count]['username'].'">'.$fav_item[$new_fav_count]['first_name'].' ' . $fav_item[$new_fav_count]['last_name'].'</a>';
                    echo ' dig this.';
            ...
            if ($fav_count >= 12) {
                ...
                echo '<p class="popup-comment" style="margin: 20px 30px 0;">
                        <a href="petstapost.html?item_id='.$fav_item[0]["item_id"].'">
                            <img src="'.$fav_item[0]['url'].'" class="comm-img '.$fav_item[0]['filter'].'">&nbsp<br>
                        </a>';
