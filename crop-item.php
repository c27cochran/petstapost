<?php
  ...
  class CropItem {
    ...
    private function setFile($file) {
      ...
      if ($errorCode === UPLOAD_ERR_OK) {
        ...
        if ($type) {
          ...
          $src = $dir . '/' . $_POST['username'] . '-item' . $extension;
    ...
    private function setDst() {
      ...
      if (file_exists($dir . '/' . $_POST['username'] . '-item' . '.png')) {
        unlink($dir . '/' . $_POST['username'] . '-item' . '.png');
      }

      $this -> dst = $dir . '/' . $_POST['username'] . '-item' . '.png';
    }
  ...
  $crop = new CropItem($_POST['item_src'], $_POST['item_data'], $_FILES['item_file']); 

  $_SESSION['item_img_url'] = $crop -> getResult();

  $response = array(
    'state'  => 200,
    'message' => $crop -> getMsg(),
    'result' => $_SESSION['item_img_url'],
    'itemname' => time().$_FILES["item_file"]["name"]
  );
