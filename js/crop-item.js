(function (factory) {
  ...
  function ItemCover($element) {
    ...
    $('#item-input').change(function(){
      ...
      $('.video-save').on('click', function(e) {
        ...
        $('#script-container').html('<script>$(".video-upload").transloadit({wait:!0,triggerUploadOnFileSelection:!0,params:{auth:{key:"confidential"},template_id: "7ae79760ee8011e4a145b969e2410dd0", redirect_url: "http://petstapost.com/yappy.php?user='+username+'&video",steps:{store_video: {path: "'+username+'/'+filename+'"},store_thumb: {path: "'+username+'/'+thumb_filename+'"},iphone_video:{use:":original",robot:"/video/encode",preset:"iphone",ffmpeg:{b:"1200k"}},extracted_thumbs:{use:"iphone_video",robot:"/video/thumbs",count:1,width:640,height:640}}}});</script>');
      });
    });
  }
  ...
  ItemCover.prototype = {
    ...
    cropDone: function () {
      ...
      this.$item.attr('src', this.url + "?" + today);
      this.stopCropper();
      ...
      $('#script-container').html("<script>$('.item-pic-upload').transloadit({wait: true, params: {auth: { key: 'confidential' }, template_id: '9b2c9990db9c11e49386c106d2f35d88', redirect_url: 'http://petstapost.com/yappy.php?user="+username+"',steps: {store: {path: '"+username+"/"+filename+"'}, import: {robot: '/http/import',url: 'http://petstapost.com/"+img_url+"'}}}});</script>");
    },
