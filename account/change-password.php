<?php
    ...
    if (empty($_GET['change-current']) || empty($_GET['change-new']) || empty($_GET['change-new-again'])) {
        ...
        $errors[] = '<i class="fa fa-exclamation-circle"></i>&nbsp;All fields are required';

    } else if ($bcrypt->verify($_GET['change-current'], $user['password']) === true) {

        if (trim($_GET['change-new']) != trim($_GET['change-new-again'])) {
            $errors[] = '<i class="fa fa-exclamation-circle"></i>&nbsp;Your new passwords do not match';
        } else if (strlen($_GET['change-new']) < 6) { 
            $errors[] = '<i class="fa fa-exclamation-circle"></i>&nbsp;Your password must be at least 6 characters';
        } else if (strlen($_GET['change-new']) >18){
            $errors[] = '<i class="fa fa-exclamation-circle"></i>&nbsp;Your password cannot be more than 18 characters long';
        } 

    } else {

        $errors[] = '<i class="fa fa-exclamation-circle"></i>&nbsp;Your current password is incorrect';
